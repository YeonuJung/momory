...
export async function GET(request: NextRequest) {
  ...
  if (redirect_uri) {
    ...
    return NextResponse.redirect(
      `https://accounts.google.com/o/oauth2/v2/auth?client_id=${GOOGLE_CLIENT_ID}&redirect_uri=${GOOGLE_REDIRECT_URI}&response_type=code&scope=email%20profile&state=${encoded_redirect_uri}`,
    );
  }
  return NextResponse.redirect(
